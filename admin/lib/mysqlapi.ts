...
// whatever they do. Against the database each duplicate is a real query, and that
// burst is what the 502s on dashboard load came out of.
//
...
// cache that could serve stale data.
//
...
export const mysqlAdmin = {
  ...
  // and a caller that already knows it is not an admin goes there directly
  // rather than collecting the 403 first.
  getContent:              async (admin = true) => {
    if (!admin) return apiFetch<AR>('/content');
    try {
