...
import { Course, SubscriberItem } from '../../types';
...
interface Props {
  ...
  setShowGrantFromCourses: (b: boolean) => void;
  grantFromCourseId: string;
  setGrantFromCourseId: (s: string) => void;
  getCourseLectures: (courseId: string) => unknown[];
  getPreset: (courseId: string) => { p1: number; p2: number };
  ...
  setAccessPresets: React.Dispatch<React.SetStateAction<Record<string, { p1: number; p2: number }>>>;
  canManageCourseAccess: boolean;
  applyAccessLevel: (courseId: string, mode: 'full' | 'limited', lectureLimit?: number) => void;
  isAdmin: boolean;
}

// Courses + access-management tab — extracted verbatim from UnifiedClientPage (Refactor #1).
const CoursesAccessTab: React.FC<Props> = ({
  subscriber, courses, updateSubscriber, showGrantFromCourses, setShowGrantFromCourses,
  grantFromCourseId, setGrantFromCourseId, getCourseLectures, getPreset, accessSaving,
  accessMsg, manualLimitDraft, setManualLimitDraft, setAccessPresets, canManageCourseAccess,
  applyAccessLevel, isAdmin,
}) => (
      ...
      <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => { setShowGrantFromCourses(false); setGrantFromCourseId(''); }}>
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-5" dir="rtl" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex gap-2">
            <button
              disabled={!grantFromCourseId}
              onClick={() => {
                if (!grantFromCourseId) return;
                updateSubscriber({ ...subscriber, enrolledCourseIds: [...subscriber.enrolledCourseIds, grantFromCourseId] });
                void mysqlAdmin.addEnrollment(subscriber.id, grantFromCourseId, null, 'full').catch(() => {});
    ...
    {subscriber.enrolledCourseIds.length === 0 ? (
      ...
      return (
        <div key={courseId} className="border border-gray-200 rounded-xl p-4">
          ...
          {isAdmin && (
          <button onClick={() => updateSubscriber({ ...subscriber, enrolledCourseIds: subscriber.enrolledCourseIds.filter(x => x !== courseId) })}
            className="mt-2 text-xs text-red-400 hover:text-red-600 hover:underline">إزالة من الاشتراك</button>
          )}
        </div>
      );
    })}
...
export default CoursesAccessTab;
