...
import type { Therapist, TherapistAvailabilitySlot, ConsultationItem } from '../../../../types';
...
interface Props {
  ...
  setIsTherapistFormOpen: React.Dispatch<React.SetStateAction<boolean>>;
  editingTherapistId: string;
  setEditingTherapistId: React.Dispatch<React.SetStateAction<string>>;
  therapistDraft: Therapist;
  setTherapistDraft: React.Dispatch<React.SetStateAction<Therapist>>;
...
/** إدارة المحاضرين — extracted from CoursesTab. */
export default function InstructorsManager(p: Props) {
  const {
    isAdmin, therapists, consultations, isTherapistFormOpen, setIsTherapistFormOpen,
    editingTherapistId, setEditingTherapistId, therapistDraft, setTherapistDraft,
    onImageUpload, saveTherapist, startEditTherapist, deleteTherapist, updateTherapist,
  } = p;
    ...
    <article className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => {
              if (isTherapistFormOpen && !editingTherapistId) { setIsTherapistFormOpen(false); return; }
      ...
      {isTherapistFormOpen && (
        <div className="border border-gray-200 rounded-2xl p-4 mb-4 bg-gray-50/70 space-y-4">
          <input ref={therapistImageInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { onImageUpload(e.target.files); e.target.value = ''; }} />
