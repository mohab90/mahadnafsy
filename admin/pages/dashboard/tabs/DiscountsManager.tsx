...
import type { Course, Bundle, Therapist, DiscountRule } from '../../../types';

type NotifyFn = (type: 'success' | 'error' | 'info', text: string) => void;
type DiscountDraft = Omit<DiscountRule, 'id' | 'createdAt'>;
...
type PromoForm = { code: string; discount_type: 'percent' | 'fixed'; discount_value: number; min_order_amount: number; max_uses: string; expires_at: string };
type D<T> = React.Dispatch<React.SetStateAction<T>>;

interface Props {
  discounts: DiscountRule[];
  editingDiscountId: string; setEditingDiscountId: (v: string) => void;
  discountDraft: DiscountDraft; setDiscountDraft: D<DiscountDraft>;
  addDiscount: (d: DiscountRule) => void;
  updateDiscount: (d: DiscountRule) => void;
  deleteDiscount: (id: string) => void;
  courses: Course[];
  bundles: Bundle[];
  therapists: Therapist[];
  policyDrafts: Record<string, string>; setPolicyDrafts: D<Record<string, string>>;
  content: Record<string, string>;
  setContentValue: (key: string, value: string) => void;
  promoCodes: PromoCode[];
  promoLoading: boolean;
  promoForm: PromoForm; setPromoForm: D<PromoForm>;
  promoFormOpen: boolean; setPromoFormOpen: D<boolean>;
  loadPromoCodes: () => void;
  createPromoCode: (payload: Record<string, unknown>) => Promise<unknown>;
  updatePromoCode: (id: string, payload: Record<string, unknown>) => Promise<unknown>;
  deletePromoCode: (id: string) => Promise<unknown>;
  notify: NotifyFn;
}

/** الخصومات والكوبونات manager — extracted from CoursesTab. */
export default function DiscountsManager(p: Props) {
  const {
    discounts, editingDiscountId, setEditingDiscountId, discountDraft, setDiscountDraft,
    addDiscount, updateDiscount, deleteDiscount, courses, bundles, therapists,
    policyDrafts, setPolicyDrafts, content, setContentValue,
    promoCodes, promoLoading, promoForm, setPromoForm, promoFormOpen, setPromoFormOpen,
    loadPromoCodes, createPromoCode, updatePromoCode, deletePromoCode, notify,
  } = p;
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            ...
            <select value={discountDraft.type} onChange={e => setDiscountDraft(p2 => ({ ...p2, type: e.target.value as DiscountRule['type'], targetId: '' }))}
          ...
          {(discountDraft.type === 'course') && (
            <div>
              ...
              <select value={discountDraft.targetId || ''} onChange={e => setDiscountDraft(p2 => ({ ...p2, targetId: e.target.value }))}
                className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400">
          ...
          <div>
            ...
            <input type="number" min={1} max={100} value={discountDraft.discountPercent}
              onChange={e => setDiscountDraft(p2 => ({ ...p2, discountPercent: Number(e.target.value) }))}
              className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" />
          </div>
          <div>
            ...
            <input type="text" placeholder="مثال: PSYCH20" value={discountDraft.promoCode || ''}
              onChange={e => setDiscountDraft(p2 => ({ ...p2, promoCode: e.target.value.toUpperCase() }))}
          ...
          <div>
            ...
            <input type="text" placeholder="مثال: خصم العيد" value={discountDraft.label || ''}
              onChange={e => setDiscountDraft(p2 => ({ ...p2, label: e.target.value }))}
              className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" />
          </div>
          <div>
            ...
            <input type="date" value={discountDraft.expiresAt || ''}
              onChange={e => setDiscountDraft(p2 => ({ ...p2, expiresAt: e.target.value }))}
              className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" />
          </div>
          <div className="flex items-center gap-2 pt-5">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={discountDraft.active} onChange={e => setDiscountDraft(p2 => ({ ...p2, active: e.target.checked }))} className="w-4 h-4 rounded" />
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              <div>
                ...
                <input type="text" value={promoForm.code} onChange={e => setPromoForm(p2 => ({ ...p2, code: e.target.value.toUpperCase().replace(/\s/g, '') }))}
              ...
              <div>
                ...
                <select value={promoForm.discount_type} onChange={e => setPromoForm(p2 => ({ ...p2, discount_type: e.target.value as 'percent' | 'fixed' }))}
              ...
              <div>
                ...
                <input type="number" min={1} value={promoForm.discount_value} onChange={e => setPromoForm(p2 => ({ ...p2, discount_value: Number(e.target.value) }))}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">الحد الأدنى للطلب (EGP)</label>
                <input type="number" min={0} value={promoForm.min_order_amount} onChange={e => setPromoForm(p2 => ({ ...p2, min_order_amount: Number(e.target.value) }))}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">أقصى استخدام (فارغ = بلا حد)</label>
                <input type="number" min={1} value={promoForm.max_uses} onChange={e => setPromoForm(p2 => ({ ...p2, max_uses: e.target.value }))}
              ...
              <div>
                ...
                <input type="date" value={promoForm.expires_at} onChange={e => setPromoForm(p2 => ({ ...p2, expires_at: e.target.value }))}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400" />
              </div>
            </div>
            <div className="flex gap-2 mt-3">
              <button
                onClick={async () => {
                  ...
                  try {
                    await createPromoCode({ code: promoForm.code, discount_type: promoForm.discount_type, discount_value: promoForm.discount_value, min_order_amount: promoForm.min_order_amount, max_uses: promoForm.max_uses ? Number(promoForm.max_uses) : null, expires_at: promoForm.expires_at || null });
                    ...
                    <tr key={pc.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                      ...
                      <td className="py-3">
                        <div className="flex gap-2">
                          <button
                            onClick={async () => {
                              try { await updatePromoCode(pc.id, { active: pc.active ? 0 : 1 }); loadPromoCodes(); }
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try { await deletePromoCode(pc.id); notify('success', 'تم حذف الكوبون'); loadPromoCodes(); }
                              catch (err) { notify('error', err instanceof Error ? err.message : 'خطأ'); }
                            }}
