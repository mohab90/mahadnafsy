...
import type { NotifyFn, StaffMember, SubscriberItem } from '../../../types';
...
export default function StaffSettingsTab({ notify, currentStaff, salesOwnSubscribers }: Props) {
  ...
  const [myHrData, setMyHrData] = useState<HrData | null>(null);
  ...
  useEffect(() => {
    setLoadingMyHr(true);
    Promise.all([
      fetch(`/api/admin/hr/employees/${currentStaff.id}`, { credentials: 'include' }).then(r => r.json()).catch(() => null),
      fetch(`/api/admin/hr/advances?staff_id=${currentStaff.id}`, { credentials: 'include' }).then(r => r.json()).catch(() => []),
  ...
  const monthlyTarget = Math.max(1, Number(draft.monthlyTarget) || 10);
  const now = new Date();
  const currentMonth = now.toISOString().slice(0, 7);
  const thisMonthSubs = salesOwnSubscribers.filter(s => (s.createdAt || '').slice(0, 7) === currentMonth);
  const achieved = thisMonthSubs.length;
  const pct = Math.min(100, Math.round((achieved / monthlyTarget) * 100));

  const monthRevenue = thisMonthSubs.reduce((total, s) => {
    const paid = (s.paymentHistory || [])
      .filter(p => (p.at || '').slice(0, 7) === currentMonth)
      .reduce((sum, p) => {
  ...
  const totalRevenue = salesOwnSubscribers.reduce((total, s) => {
  ...
  for (let w = 0; w < 4; w++) {
    ...
    const count = salesOwnSubscribers.filter(s => {
      const d = s.createdAt ? new Date(s.createdAt) : null;
      if (!d) return false;
      return d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth() && d.getDate() >= weekStart && d.getDate() <= weekEnd;
    }).length;
    weeksData.push({ label: `أسبوع ${w + 1}`, count });
  }
                              ...
                              <div className="flex gap-2">
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      const r = await fetch('/api/admin/hr/leaves', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ staff_id: currentStaff.id, ...myLeaveFormProfile }) });
                                      ...
                                      const hrData = await fetch(`/api/admin/hr/employees/${currentStaff.id}`, { credentials: 'include' }).then(r2 => r2.json()).catch(() => null);
                                      if (hrData && !hrData.error) setMyHrData(hrData);
                              ...
                              <div className="flex gap-2">
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      const r = await fetch('/api/admin/hr/advances', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ staff_id: currentStaff.id, amount: Number(advanceDraft.amount), reason: advanceDraft.reason }) });
                                      if (!r.ok) throw new Error();
