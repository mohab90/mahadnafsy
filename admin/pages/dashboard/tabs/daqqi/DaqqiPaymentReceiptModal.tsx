...
import type { DaqqiPayPrintData } from './useDaqqiPaymentState';

interface DaqqiPaymentReceiptModalProps {
  data: DaqqiPayPrintData | null;
  content: Record<string, string>;
  onClose: () => void;
}

export const DaqqiPaymentReceiptModal: React.FC<DaqqiPaymentReceiptModalProps> = ({
  data,
  content,
  onClose,
}) => {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[320px]" dir="rtl" onClick={e => e.stopPropagation()}>
        <div className="bg-gray-800 text-white rounded-t-2xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => window.print()} className="px-3 py-1.5 bg-white text-gray-800 rounded-lg text-xs font-bold hover:bg-gray-100 transition">طباعة</button>
        ...
        <div id="daqqiPrintReceipt" className="p-4 font-mono text-[11px] leading-relaxed" style={{ width: '80mm', boxSizing: 'border-box', fontFamily: 'monospace' }}>
          ...
          <div className="space-y-0.5">
            {data.items.map((item, i) => (
              <div key={`${item.label}-${i}`} className="flex justify-between">
          ...
          <div className="space-y-0.5">
            ...
            {data.prevPaid > 0 && (
              <div className="flex justify-between"><span className="text-gray-600">مدفوع سابقا:</span><span className="font-bold">{data.prevPaid.toLocaleString()} {data.currency}</span></div>
            )}
          ...
          <div className="text-center text-[10px] text-gray-500 space-y-0.5">
            ...
            <div className="mt-1">شكرا لثقتكم</div>
          </div>
        </div>
      </div>
    </div>
