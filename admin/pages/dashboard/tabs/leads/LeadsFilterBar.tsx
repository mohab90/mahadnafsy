...
export type FollowupFilter = 'all' | 'today' | 'overdue' | 'past3d' | 'past7d' | 'past30d' | 'next3d' | 'next7d' | 'no_followup';

interface LeadsFilterBarProps {
  isSalesOnly: boolean;
  searchTerm: string;
  setSearchTerm: (v: string) => void;
  assignedReps: { id: string; name: string }[];
  assignFilter: Set<string>;
  setAssignFilter: (v: Set<string>) => void;
  singleStatus: LeadStatus | '';
  setSingleStatus: (v: LeadStatus | '') => void;
  courses: Course[];
  bundles: Bundle[];
  ...
  setBranchFilter: (v: string | null) => void;
  effectiveLeads: LeadItem[];
  salesSourceFilter: string;
  setSalesSourceFilter: (v: string) => void;
  leadsFollowupFilter: FollowupFilter;
  setLeadsFollowupFilter: (v: FollowupFilter) => void;
  showHiddenLeads: boolean;
  setShowHiddenLeads: React.Dispatch<React.SetStateAction<boolean>>;
  totalConverted: number;
  totalLost: number;
  visibleLeadsCount: number;
}

// Shared filter bar for the pipeline + table sub-tabs. Extracted verbatim from LeadsTab.
export function LeadsFilterBar({
  isSalesOnly, searchTerm, setSearchTerm, assignedReps, assignFilter, setAssignFilter,
  singleStatus, setSingleStatus, courses, bundles, courseFilter, setCourseFilter,
  instituteBranches, branchFilter, setBranchFilter, effectiveLeads, salesSourceFilter,
  setSalesSourceFilter, leadsFollowupFilter, setLeadsFollowupFilter, showHiddenLeads,
  setShowHiddenLeads, totalConverted, totalLost, visibleLeadsCount,
}: LeadsFilterBarProps) {
  return (
