    ...
    <section className="space-y-4">
      ...
      {data && <>
        <div className={`flex items-center gap-3 rounded-2xl border p-4 ${data.ok ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
          ...
          <div><div className="font-extrabold text-gray-900">{data.ok ? 'المطابقة سليمة' : `${data.criticalCount} مشكلة حرجة تحتاج مراجعة`}</div><div className="text-xs text-gray-500">آخر فحص: {new Date(data.checkedAt).toLocaleString('ar-EG-u-nu-latn', { timeZone: CAIRO_TIME_ZONE })}</div></div>
        </div>
