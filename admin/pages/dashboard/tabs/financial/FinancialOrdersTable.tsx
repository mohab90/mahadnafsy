...
function printOnline(row: FinancialOrderRow) {
  ...
  const w = window.open('', '_blank', 'width=700,height=900');
  if (!w) return;
  w.document.write(`<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>فاتورة #${escapeHtml(r.id)}</title><style>body{font-family:Arial,sans-serif;padding:40px;direction:rtl;color:#111}.header{text-align:center;border-bottom:3px solid #7c3aed;padding-bottom:20px;margin-bottom:30px}h1{color:#7c3aed;margin:0;font-size:24px}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{padding:12px 16px;border:1px solid #e5e7eb;text-align:right}th{background:#f9fafb;font-weight:700}tfoot td{font-weight:700;background:#f0fdf4}.footer{margin-top:40px;text-align:center;color:#888;font-size:12px;border-top:1px solid #e5e7eb;padding-top:16px}</style></head><body><div class="header"><h1>معهد الدراسات النفسية</h1><p style="color:#888;font-size:12px">فاتورة رقم: ${escapeHtml(r.id)} — ${escapeHtml((r.paidAt||r.createdAt||'').slice(0,10))}</p></div><h3>${escapeHtml(r.customerName)}</h3><table><thead><tr><th>الخدمة</th><th>طريقة الدفع</th><th>المبلغ</th></tr></thead><tbody><tr><td>${escapeHtml(r.itemTitle)}</td><td>${r.paymentMethod==='card'?'بطاقة بنكية':'محفظة إلكترونية'}</td><td>${escapeHtml(r.amount)} ${escapeHtml(r.currency)}</td></tr></tbody><tfoot><tr><td colspan="2">الإجمالي</td><td>${escapeHtml(r.amount)} ${escapeHtml(r.currency)}</td></tr></tfoot></table><div class="footer">معهد الدراسات النفسية — mahadnafsy.com</div></body></html>`);
  w.document.close();
  setTimeout(() => w.print(), 500);
}
