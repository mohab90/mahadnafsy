...
import { type LeadItem, type LeadStatus, type StaffMember } from '../../types';
...
interface Props {
  ...
  setLeadsFollowupFilter: (f: string) => void;
  setActiveTab: (tab: TabKey) => void;
}

export default function SalesFollowupPanel({
  salesNotifOpen,
  setSalesNotifOpen,
  isNonAdminStaff,
  isAdmin,
  ...
  if (!salesNotifOpen) return null;

  const todayStr = new Date().toISOString().slice(0, 10);
  const myLeads = isNonAdminStaff && currentStaff
    ? salesOwnLeads.filter(l => !['converted', 'lost'].includes(l.status))
      ...
      <div className="bg-white h-full w-full max-w-md shadow-2xl flex flex-col" dir="rtl" onClick={e => e.stopPropagation()}>
        ...
        <div className="border-t border-gray-200 px-5 py-4 bg-gray-50 space-y-2">
          <div className="flex gap-2">
            <button
              onClick={() => { setLeadsFollowupFilter('overdue'); setSalesNotifOpen(false); setActiveTab('leads'); }}
            ...
            <button
              onClick={() => { setLeadsFollowupFilter('today'); setSalesNotifOpen(false); setActiveTab('leads'); }}
