...
interface UnifiedClientCommunicationsPanelProps {
  communications: UnifiedCommunication[];
  clientName: string;
  clientPhone: string;
  isSubscriber: boolean;
  onAdd: () => void;
  onDelete: (id: string, source: 'lead' | 'subscriber') => void;
}

export function UnifiedClientCommunicationsPanel({
  communications,
  clientName,
  clientPhone,
  isSubscriber,
  onAdd,
  onDelete,
}: UnifiedClientCommunicationsPanelProps) {
  return (
    <div id="section-communications" className="space-y-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-[11px] font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{communications.length}</span>
      </div>
                ...
                <div key={comm.id} className="flex gap-4 group">
                  ...
                  <div className="flex-1 bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button
                        onClick={() => onDelete(comm.id, source)}
                        className="text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
                    {comm.notes && <p className="text-sm text-gray-700 mt-2 leading-relaxed">{comm.notes}</p>}
                    {comm.outcome && <div className="mt-2 flex items-center gap-2 text-xs text-green-700 bg-green-50 px-3 py-1.5 rounded-lg"><CheckCircle size={12} />{comm.outcome}</div>}
                    {comm.nextFollowUp && <div className="mt-1 flex items-center gap-2 text-xs text-orange-700 bg-orange-50 px-3 py-1.5 rounded-lg"><Clock size={12} />موعد المتابعة: {comm.nextFollowUp}</div>}
                  </div>
                </div>
              );
            })}
