  ...
  return (
    <div className="mb-3 grid grid-cols-3 gap-2">
      <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-center">
        ...
        <p className="text-base font-extrabold text-emerald-700">{totalPaidEGP.toLocaleString('ar-EG-u-nu-latn')} {settlementLabel}</p>
      </div>
      <div className={`rounded-xl border p-3 text-center ${remainingEGP > 0 ? 'border-red-100 bg-red-50' : 'border-gray-100 bg-gray-50'}`}>
        ...
        <p className={`text-base font-extrabold ${remainingEGP > 0 ? 'text-red-600' : 'text-gray-400'}`}>
          {remainingEGP > 0 ? `${remainingEGP.toLocaleString('ar-EG-u-nu-latn')} ${settlementLabel}` : '✓'}
        </p>
      </div>
