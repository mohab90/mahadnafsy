...
export const commTypeMeta: Record<string, { label: string; color: string; icon: string }> = {
  ...
  whatsapp: { label: 'واتساب', color: 'bg-green-100 text-green-700', icon: '💬' },
...
export const normalizeAccess = (entry?: CourseAccessSetting | 'preview' | 'full'): CourseAccessSetting => {
  ...
  if (entry.mode === 'limited') {
    const limit = Number(entry.lectureLimit || 1);
    return { mode: 'limited', lectureLimit: Number.isFinite(limit) && limit > 0 ? Math.floor(limit) : 1 };
  }
