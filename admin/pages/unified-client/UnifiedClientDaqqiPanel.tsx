...
const roundStatusClass: Record<DaqqiRound['status'], string> = {
  new: 'bg-blue-100 text-blue-700',
  active: 'bg-green-100 text-green-700',
  finished: 'bg-gray-100 text-gray-600',
};

const roundStatusLabel: Record<DaqqiRound['status'], string> = {
  new: 'جديدة',
  active: 'جارية',
  finished: 'منتهية',
};

export function UnifiedClientDaqqiPanel({ rounds, courses, subscriber }: UnifiedClientDaqqiPanelProps) {
  return (
    <div id="section-daqqi" className="space-y-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-[11px] font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{rounds.length}</span>
      </div>
        ...
        return (
          <div key={round.id} className="border border-gray-200 rounded-xl p-4 bg-white shadow-sm">
            <div className="flex items-start justify-between gap-2 mb-3">
              ...
              <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${roundStatusClass[round.status] || 'bg-gray-100 text-gray-600'}`}>
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                ...
                <div className={`rounded-lg p-2 text-center border ${remaining > 0 ? 'bg-red-50 border-red-100' : 'bg-green-50 border-green-100'}`}>
                  {remaining > 0 ? (
                    ...
                    <>
                      <p className="font-bold text-green-700 text-sm">✅</p>
                      <p className="text-[10px] text-gray-400">مكتمل</p>
                    </>
                  )}
                </div>
