...
const CourseCard: React.FC<CourseCardProps> = ({ course, currency }) => {
  ...
  // Display student count: 200 base + real enrolled count
  const displayStudents = (course.students || 0) + 200;

  // Deterministic varied rating (4.6–4.9) per course based on its ID
  const ratingPool = [4.9, 4.8, 4.7, 4.6, 4.8, 4.9, 4.7, 4.8];
  const hash = course.id.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);
  const displayRating =
    course.rating >= 4.6 && course.rating <= 4.9
      ? course.rating
      : ratingPool[hash % ratingPool.length];
  // Use DOM parser to extract plain text from any HTML (handles spans, encoded entities, etc.)
    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 border border-gray-100 flex flex-col h-full">
      ...
      <div className="p-5 flex-grow flex flex-col">
        ...
        <div className="mt-auto border-t pt-4 flex justify-between items-center">
          ...
          <Link to={`/course/${course.id}`} className="bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition">
            التفاصيل
          </Link>
        </div>
      </div>
