 ...
 *
 * The leads import used to split on '\n' and then on ',', which cannot read
 * what this module writes: a comma inside a quoted note opened an extra
 * column, a doubled quote «""» toggled twice and vanished, and a note with a
 * newline in it became two half rows. Exporting the leads and importing them
 * again was enough to corrupt them.
 *
 * Unquoted fields are trimmed, which is what the import UI has always done;
 * a quoted field is kept exactly as written, because the quotes are how the
 * writer says the whitespace was meant.
 */
