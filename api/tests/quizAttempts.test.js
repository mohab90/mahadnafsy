...
const quiz = {
  ...
  questions_json: JSON.stringify([
    { id: 'q1', options: ['a', 'b', 'c', 'd'], correctIndex: 0 },
...
test('recordQuizAttempt: writes a real quiz_attempts row scored against the server-held quiz (LMS-06)', async () => {
  ...
  const result = await recordQuizAttempt({
    quiz, subscriberId: 'sub-1', answers: [0, 1], tenantId: 't1', actor: 'student@x.com',
    completionService: async () => ({ certificate_code: 'MHAD-TEST' }),
  }, db);
...
test('recordQuizAttempt: never trusts a client-supplied score — grading always comes from the server copy of correctIndex', async () => {
  ...
  const result = await recordQuizAttempt({
    quiz, subscriberId: 'sub-1', answers: [1, 0], tenantId: 't1', actor: 'student@x.com',
  }, db);
  assert.equal(result.score, 0);
  assert.equal(result.passed, false);
});

test('recordQuizAttempt rejects malformed answers before writing an attempt', async () => {
  const db = mockDb([{ match: 'INSERT INTO quiz_attempts', rows: {} }]);
  await assert.rejects(
    recordQuizAttempt({
      quiz, subscriberId: 'sub-1', answers: [9, 9], tenantId: 't1', actor: 'student@x.com',
    }, db),
    (error) => error.statusCode === 400 && /valid option/.test(error.message)
  );
