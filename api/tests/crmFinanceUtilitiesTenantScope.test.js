...
test('public lead capture is tenant-deduped, serialized, assigned and audited atomically', () => {
  ...
  // `RIGHT(REGEXP_REPLACE(phone,...),10)=RIGHT(?,10)`, which is a tail match and
  ...
  // is that the dedup is tenant-scoped and exact — lib/leadMatching.js owns how,
  // so the clause is matched by shape rather than by the variable holding it.
  assert.match(route, /FROM leads WHERE tenant_id=\? AND \$\{\w+\.sql\}/);
