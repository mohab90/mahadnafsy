...
router.patch('/api/me/progress', requireAuth, async (req, res) => {
  try {
    ...
    const {
      ...
      watchSeconds: legacyWatchSeconds,
      lecture_id,
      course_id,
      progress_pct,
      watch_seconds,
    } = req.body || {};
    const lectureId = legacyLectureId || lecture_id;
    const pct = legacyPct ?? progress_pct;
    const suppliedCourseId = legacyCourseId || course_id || null;
    ...
    const [[lec]] = await pool.query('SELECT course_id FROM course_lectures WHERE id=? LIMIT 1', [lectureId]).catch(() => [[null]]);
    const courseId = suppliedCourseId || lec?.course_id || null;
    if (courseId) {
      await pool.query(
        `INSERT INTO lecture_completions (id, subscriber_id, lecture_id, course_id, progress_pct, watch_seconds, completed_at)
         VALUES (UUID(),?,?,?,?,?,IF(? >= 90, NOW(), NULL))
         ON DUPLICATE KEY UPDATE
           progress_pct=GREATEST(progress_pct,VALUES(progress_pct)),
           watch_seconds=GREATEST(watch_seconds,VALUES(watch_seconds)),
           completed_at=IF(VALUES(progress_pct)>=90, COALESCE(completed_at, NOW()), completed_at)`,
        [sub.id, lectureId, courseId, progress, Number(legacyWatchSeconds ?? watch_seconds) || 0, progress]
      ).catch(error => logger.warn('lecture_completions save failed', { error: error.message, lectureId, courseId }));
    }

    let completionData = null;
    if (progress >= 90 && courseId) {
      try {
