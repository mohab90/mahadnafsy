...
router.get('/api/admin/hr/enps', requireAuth, requireAdminOrStaff, requirePermission('view_hr'), async (req, res) => {
  try {
    ...
    const where = period ? 'tenant_id=? AND period=?' : 'tenant_id=?';
    const params = period ? [req.tenantId, period] : [req.tenantId];
    const [[agg]] = await pool.query(
      `SELECT COUNT(*) total, AVG(score) avg_score, SUM(score>=9) promoters,
         ...
         FROM enps_responses WHERE ${where}`, params
    );
    const enps = agg.total > 0 ? Math.round(((agg.promoters - agg.detractors) / agg.total) * 100) : null;
    const [comments] = await pool.query(
      `SELECT score, comment, period, created_at FROM enps_responses
        WHERE ${where} AND comment IS NOT NULL AND comment <> ''
    ...
    res.json({
      enps, total: Number(agg.total) || 0,
      avg: agg.avg_score ? parseFloat((+agg.avg_score).toFixed(1)) : null,
      promoters: Number(agg.promoters) || 0, passives: Number(agg.passives) || 0, detractors: Number(agg.detractors) || 0,
      comments, periods: periods.map(p => p.period),
    });
  } catch (e) { logger.error('[hr/enps]', e.message); res.status(500).json({ error: 'Internal server error' }); }
});
