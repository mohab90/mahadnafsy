...
--   (SELECT COUNT(*) FROM subscribers WHERE tenant_id=? AND status NOT IN (...))
--   only had tenant_id indexed alone (idx_subscribers_tenant) or paired with
--   created_at — never with status.
ALTER TABLE IF EXISTS subscribers ADD INDEX IF NOT EXISTS idx_subscribers_tenant_status (tenant_id, status);
