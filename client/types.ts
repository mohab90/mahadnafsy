...
export type StaffRole = 'instructor' | 'trainer' | 'expert' | 'sales' | 'manager' | 'admin' | 'support' | 'reception_daqqi' | 'daqqi_manager' | 'collection' | 'accountant' | 'consultant' | 'other';
...
export interface SubscriberItem {
  ...
  status: 'active' | 'active_new' | 'active_paid' | 'active_late' | 'paused' | 'blocked';
  createdAt: string;
  communications?: CommunicationRecord[];
...
export interface OrderItem {
  ...
  type: 'course' | 'bundle' | 'consultation';
  itemId: string;
  itemTitle: string;
  amount: number;
  currency: 'EGP' | 'SAR' | 'USD';
